...
author: "Harry Cheng, Maggie Zhou"
format:
  dashboard:
...
We use two public datasets: [MrAirspace](https://github.com/MrAirspace/aircraft-flight-schedules) for dynamic flight activity and [OurAirports](https://ourairports.com/help/data-dictionary.html) for geographic airport reference data.
...
- Record size: The database contains detailed records for `r airports_rows` facilities globally, ensuring comprehensive analytical coverage.
...
**arrow + `read_parquet()`**\
We used `arrow` and `read_parquet()` to load the flight dataset from a Parquet file format. Source: We learned about this from the official documentation for [`read_parquet()`](https://kou.github.io/arrow-site/docs/r/reference/read_parquet.html) while learning how to import Parquet files in R.
...
Source: We learned about this from the official documentation for [`rename()`](https://dplyr.tidyverse.org/reference/rename.html) while using it to distinguish similarly named fields after joins.
...
Source: We learned about this from the official documentation for [`countrycode()`](https://vincentarelbundock.github.io/countrycode/reference/countrycode.html) while preparing country labels for maps and summaries.
...
Source: We learned about this from the official documentation for [`cumsum()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/cumsum.html) while building the cumulative distribution for global flight concentration.
...
Source: We learned about this from the official documentation for [`scale_x_continuous()` and `scale_y_continuous()`](https://ggplot2.tidyverse.org/reference/scale_continuous.html) while refining chart axes and labels.
...
Source: We learned about this from the official documentation for [`count()`](https://dplyr.tidyverse.org/reference/count.html) while looking for a shorter grouped-counting workflow.
...
Source: We learned about this from the official documentation for [`slice_max()`](https://dplyr.tidyverse.org/reference/slice.html) while selecting the highest-ranked results for one of our plots.
...
Source: We learned about this from the official documentation for [`geom_text()`](https://ggplot2.tidyverse.org/reference/geom_text.html) while adding direct labels to improve chart readability.
...
Source: We learned about this from the official documentation for [`reorder()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/reorder.factor.html) while ordering categories by data values instead of alphabetically.
...
Source: We learned about this from the official documentation for [`coord_quickmap()`](https://ggplot2.tidyverse.org/reference/coord_map.html), [`coord_fixed()`](https://ggplot2.tidyverse.org/reference/coord_fixed.html), and [`coord_flip()`](https://ggplot2.tidyverse.org/reference/coord_flip.html) while adjusting maps and plot layouts.
:::

# Conclusion + Group Info
