...
flight_df <- read_parquet("2025_Q4.parquet")
...
global_flight_distribution_plot <-
  ...
  geom_polygon(aes(x = long, y = lat, group = group, fill = total_flights)) +
...
global_total_bar_plot  <-
  ggplot(top_flight_country_df, aes(x = reorder(map_country_name, -total_flights), y = total_flights)) +
...
us_top_inter_plot <-
  ggplot(intl_top_countries, aes(x = country_name, y = total_intl, fill = total_intl)) +
  geom_col(width = 0.7) +
...
us_top_domes_plot <-
  ...
  geom_polygon(aes(x = long, y = lat, group = group, fill = dom_flight_n), 
